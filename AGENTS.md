...
- Statuses: `Pending`, `Verifikasi Doc`, `Proses Development`, `Proses BSSN`, `Proses`, `Selesai`, `Ditolak`. The admin "Proses" filter groups the three in-progress statuses via `whereIn` (routes/web.php and AdminPengajuanController) — update both if you add a status.
