            ...
            <div class="relative bg-white rounded-3xl w-full max-w-6xl mx-4 my-auto shadow-2xl border border-slate-100 overflow-hidden max-h-[92vh] flex flex-col animate-fade-in-down">
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-white shrink-0">
                    <div class="flex items-center gap-3.5 min-w-0">
                        <div class="w-11 h-11 rounded-xl bg-[#16324F] text-teal-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20 shrink-0">
                            <i class="fa-solid fa-server"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold tracking-[0.16em] text-teal-700 uppercase mb-0.5">Admin Panel</p>
                            <h2 class="text-[16px] font-extrabold text-[#101828] leading-tight truncate">Kelola Pengajuan Hosting</h2>
                            <p class="text-[11px] text-slate-500 font-medium mt-0.5">Perbarui progres, unggah dokumen akses, dan balas diskusi pemohon.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-teal-800 bg-teal-50 border border-teal-200 rounded-full px-3 py-1.5 shrink-0">
                        ...
                        <div class="p-5 bg-gradient-to-r from-slate-50 to-slate-100/70 border border-slate-200/80 rounded-2xl flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 shadow-sm">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-slate-500 font-medium mt-0.5 truncate">
                                    NIP: {{ $dataForm['nip'] ?? $item->user->nip ?? '-' }} &middot; {{ $dataForm['instansi'] ?? $item->user->unit_kerja ?? 'Instansi' }}
                                </p>
                            </div>
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
                                ...
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Aplikasi</p>
                                    <span class="px-3.5 py-1.5 bg-teal-100 text-teal-800 rounded-xl text-xs font-bold shadow-sm inline-block">{{ $dataForm['nama_aplikasi'] ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            @if(!empty($filePemohon))
                                <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'pendukung']) }}" target="_blank" class="px-4 py-2.5 bg-white border border-slate-200 hover:border-teal-500 hover:bg-teal-50/30 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2.5 shadow-sm transition-all shrink-0">
                                    <i class="fa-solid fa-file-pdf text-sm text-teal-600"></i> Surat Permohonan
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                <div class="space-y-4 flex-1">
                                    ...
                                    <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                        ...
                                        <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white hover:border-teal-500 hover:bg-teal-50/40 transition-all px-3 py-2.5 cursor-pointer">
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                <div class="w-8 h-8 shrink-0 rounded-lg bg-white group-hover:bg-teal-600 group-hover:text-white border border-slate-200 group-hover:border-teal-600 flex items-center justify-center text-slate-500 transition-colors shadow-sm">
                                            ...
                                            <div class="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[10.5px] font-bold text-gray-600 group-hover:border-teal-300 shrink-0">Browse</div>
