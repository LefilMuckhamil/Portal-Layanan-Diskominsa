...
<div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 overflow-hidden flex flex-col">
    ...
    @if(session('error'))
        <div class="bg-rose-50 text-rose-600 px-6 py-3 border-b border-rose-100 text-[13px] font-bold flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($users as $item)
                    <tr class="hover:bg-cyan-50/20 transition-colors duration-200">
                        <td class="py-4 px-6">
                            <p class="text-[13px] font-extrabold text-[#071E3D]">{{ $item->name }}</p>
                            <p class="text-[11px] text-gray-500 font-medium mt-0.5">{{ $item->jabatan ?? '-' }}</p>
                        </td>
                        <td class="py-4 px-6 text-[12px] font-bold text-gray-600">{{ $item->email }}</td>
                        <td class="py-4 px-6 text-[12px] font-bold text-gray-500">{{ $item->nip ?? '-' }}</td>
                        <td class="py-4 px-6 text-[12px] font-bold text-gray-500">{{ $item->unit_kerja ?? '-' }}</td>
                        <td class="py-4 px-6 text-[12px] font-bold text-gray-600">{{ $item->no_hp ?? '-' }}</td>
                        <td class="py-4 px-6">
                            @if($item->role === 'admin')
                        ...
                        <td class="py-4 px-6 text-center space-x-1 whitespace-nowrap">
                            <button type="button" onclick="bukaModalEdit('{{ $item->id }}')" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:bg-cyan-50 hover:text-cyan-600 border border-gray-200 transition-colors inline-flex items-center justify-center shadow-sm" title="Edit Akun">
                    ...
                    <div id="modal-edit-{{ $item->id }}" class="fixed inset-0 z-[150] hidden items-center justify-center">
                        ...
                        <div class="relative bg-white rounded-[1.5rem] w-full max-w-3xl mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col animate-fade-in-down">
                            ...
                            <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar">
                                <form method="POST" action="{{ route('admin.users.update', $item->id) }}" onsubmit="disableSubmitButton(this)">
                                    @csrf
                                    @method('PUT')
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                                        <div class="col-span-1 md:col-span-2">
                                            ...
                                            <input type="text" name="name" value="{{ old('name', $item->name) }}" required class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Email Resmi <span class="text-rose-500">*</span></label>
                                            <input type="email" name="email" value="{{ old('email', $item->email) }}" required class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">NIP</label>
                                            <input type="text" name="nip" value="{{ old('nip', $item->nip) }}" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Unit Kerja</label>
                                            <input type="text" name="unit_kerja" value="{{ old('unit_kerja', $item->unit_kerja) }}" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Jabatan</label>
                                            <input type="text" name="jabatan" value="{{ old('jabatan', $item->jabatan) }}" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">No. HP / WhatsApp <span class="text-rose-500">*</span></label>
                                            <input type="text" name="no_hp" value="{{ old('no_hp', $item->no_hp) }}" required class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                        <div>
                                            <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Role <span class="text-rose-500">*</span></label>
                                            <select name="role" required class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-bold appearance-none cursor-pointer">
                                        ...
                                        <div class="col-span-1 md:col-span-2">
                                            ...
                                            <input type="password" name="password" placeholder="Kosongkan jika tidak ingin mengubah password" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                                        </div>
                                    </div>
...
<div id="modal-create" class="fixed inset-0 z-[150] hidden items-center justify-center">
    ...
    <div class="relative bg-white rounded-[1.5rem] w-full max-w-3xl mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col animate-fade-in-down">
        ...
        <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar">
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" onsubmit="disableSubmitButton(this)">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Nama Lengkap <span class="text-rose-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama lengkap..." class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Email Resmi <span class="text-rose-500">*</span></label>
                        <input type="email" name="email" value="{{ old('email') }}" required placeholder="nama@acehbaratkab.go.id" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">NIP</label>
                        <input type="text" name="nip" value="{{ old('nip') }}" placeholder="18 digit" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Unit Kerja</label>
                        <input type="text" name="unit_kerja" value="{{ old('unit_kerja') }}" placeholder="Dinas / Instansi" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Jabatan</label>
                        <input type="text" name="jabatan" value="{{ old('jabatan') }}" placeholder="Jabatan / Fungsional" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">No. HP / WhatsApp <span class="text-rose-500">*</span></label>
                        <input type="text" name="no_hp" value="{{ old('no_hp') }}" required placeholder="08xxxxxxxxxx" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                    <div>
                        <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Role <span class="text-rose-500">*</span></label>
                        <select name="role" required class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-bold appearance-none cursor-pointer">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="password" name="password" required placeholder="Minimal 8 karakter" class="dk-input-modal w-full px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                    </div>
                </div>
