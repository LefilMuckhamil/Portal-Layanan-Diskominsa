<div data-ajax-table class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 overflow-hidden flex flex-col mt-6">
    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-lg font-extrabold text-[#071E3D]">Daftar Pengajuan Subdomain</h3>
            <p class="text-[12px] text-gray-400 font-medium mt-1">Kelola data dan perbarui status progres pembuatan subdomain.</p>
        </div>

        <div class="flex gap-3 items-center">
            <form method="GET" action="{{ route('admin.subdomain.index') }}" class="flex gap-3">
                <div class="relative">
                    <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-[11px]"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Tiket #SUB-..." class="pl-8 pr-4 py-2 bg-gray-50 border border-gray-100 rounded-lg text-[12px] font-bold text-gray-600 outline-none focus:border-violet-400 focus:bg-white w-48 transition-all">
                </div>
                <div class="relative">
        ...
        <table class="w-full text-left border-collapse min-w-[900px]">
            <thead class="bg-gray-50/50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-400 font-bold">
                <tr>
                    ...
                    <th class="py-3 px-6">Subdomain</th>
                    <th class="py-3 px-6">IP & Aplikasi</th>
            ...
            <tbody id="admin-tbody" class="divide-y divide-gray-50">
                @forelse ($pengajuans as $item)
                    ...
                    @endphp
                    <tr class="hover:bg-violet-50/20 transition-colors duration-200">
                        <td class="py-4 px-6 text-[13px] font-extrabold text-[#071E3D] whitespace-nowrap font-mono">
                            <span class="inline-flex items-center gap-1.5">
                                {{ $item->nomor_tiket }}
                                <button type="button" onclick="event.stopPropagation(); copyTiket('{{ $item->nomor_tiket }}')" title="Salin Nomor Tiket" class="text-gray-400 hover:text-violet-500 transition-colors cursor-pointer"><i class="fa-regular fa-copy text-[11px]"></i></button>
                            </span>
                        </td>
                        <td class="py-4 px-6">
                        ...
                        <td class="py-4 px-6">
                            ...
                            @if(!empty($dataForm['no_hp']))
                                <p class="text-[11px] text-emerald-600 font-medium mt-0.5">{{ \App\Support\PhoneNumber::local($dataForm['no_hp'] ?? '') }}</p>
                            @endif
                        </td>
                        <td class="py-4 px-6">
                            <span class="text-[12.5px] font-black text-violet-800 break-all">{{ $dataForm['domain'] ?? '-' }}</span>
                        </td>
                        <td class="py-4 px-6">
                            @if(!empty($dataForm['ip_address']))
                                <p class="text-[12px] font-mono font-bold text-gray-700">{{ $dataForm['ip_address'] }}</p>
                            @endif
                            @if(!empty($dataForm['nama_aplikasi']))
                                <p class="text-[11px] text-gray-500 font-medium mt-0.5">{{ $dataForm['nama_aplikasi'] }}</p>
                            @endif
                        </td>
                        <td class="py-4 px-6 text-[12px] text-gray-500 font-bold">{{ $item->created_at->format('d M Y') }}</td>
                        <td class="py-4 px-6">
                        ...
                        <td class="py-4 px-6 text-center space-x-1 whitespace-nowrap">
                            <button type="button" onclick="bukaModalAdmin('{{ $item->id }}')" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:bg-violet-50 hover:text-violet-600 border border-gray-200 transition-colors inline-flex items-center justify-center shadow-sm cursor-pointer" title="Kelola & Detail Pengajuan">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="py-12 text-center">
                            ...
                            <h3 class="font-bold text-[14px] text-gray-600">Belum ada data pengajuan subdomain</h3>
                        </td>
                    </tr>
                @endforelse
            </tbody>
            ...
            <div class="relative bg-white rounded-3xl w-full max-w-6xl mx-4 my-auto shadow-2xl border border-slate-100 overflow-hidden max-h-[92vh] flex flex-col animate-fade-in-down">
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-white shrink-0">
                    <div class="flex items-center gap-3.5 min-w-0">
                        <div class="w-11 h-11 rounded-xl bg-[#16324F] text-violet-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20 shrink-0">
                            <i class="fa-solid fa-route"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold tracking-[0.16em] text-violet-700 uppercase mb-0.5">Admin Panel</p>
                            <h2 class="text-[16px] font-extrabold text-[#101828] leading-tight truncate">Kelola Pengajuan Subdomain</h2>
                            <p class="text-[11px] text-slate-500 font-medium mt-0.5">Perbarui progres, unggah dokumen hasil, dan balas diskusi pemohon.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-violet-800 bg-violet-50 border border-violet-200 rounded-full px-3 py-1.5 shrink-0">
                            <span class="w-1.5 h-1.5 rounded-full bg-violet-500"></span> {{ $item->nomor_tiket }}
                        </span>
                    ...
                    <form method="POST" action="{{ route('admin.pengajuan.update', $item->id) }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                        ...
                        <div class="p-5 bg-gradient-to-r from-slate-50 to-slate-100/70 border border-slate-200/80 rounded-2xl flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 shadow-sm">
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Email Dinas</p>
                                    <p class="text-xs font-semibold text-slate-700 break-all max-w-[220px]">{{ $dataForm['email_dinas'] ?? '-' }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Subdomain</p>
                                    <span class="px-3.5 py-1.5 bg-violet-100 text-violet-800 rounded-xl text-xs font-bold shadow-sm inline-block">{{ $dataForm['domain'] ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            @if(!empty($filePemohon))
                                <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'pendukung']) }}" target="_blank" class="px-4 py-2.5 bg-white border border-slate-200 hover:border-violet-500 hover:bg-violet-50/30 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2.5 shadow-sm transition-all shrink-0">
                                    <i class="fa-solid fa-file-pdf text-sm text-violet-600"></i> Surat Permohonan
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                <div class="space-y-4 flex-1">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="catatan" rows="4" placeholder="Tuliskan catatan progres yang akan muncul di E-Tracking pemohon..." class="w-full rounded-xl border border-slate-300 p-3.5 text-sm focus:ring-2 focus:ring-blue-500 bg-white placeholder:text-slate-400">{{ old('catatan') }}</textarea>
                                    </div>

                                    <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Upload Dokumen Hasil / Akses (PDF)</label>
                                        <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white hover:border-violet-500 hover:bg-violet-50/40 transition-all px-3 py-2.5 cursor-pointer">
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                <div class="w-8 h-8 shrink-0 rounded-lg bg-white group-hover:bg-violet-600 group-hover:text-white border border-slate-200 group-hover:border-violet-600 flex items-center justify-center text-slate-500 transition-colors shadow-sm">
                                                    <i class="fa-solid fa-cloud-arrow-up text-[13px]"></i>
                                                </div>
                                                <div class="min-w-0">
                                                    <p id="admin-hasil-name-{{ $item->id }}" class="text-[12px] text-[#101828] font-bold group-hover:text-violet-900 truncate">
                                                        @if(!empty($dataForm['file_hasil']))
                                                            File Hasil Tersedia
                                                            <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'hasil']) }}" target="_blank" class="inline-flex items-center gap-1 ml-1.5 text-violet-700 bg-violet-100 hover:bg-violet-200 px-2 py-0.5 rounded-md text-[10px] font-bold transition-colors">
                                                    ...
                                                    <p class="text-[10px] text-slate-500">Dokumen hasil pembuatan</p>
                                                </div>
                                            </div>
                                            <div class="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[10.5px] font-bold text-gray-600 group-hover:border-violet-300 shrink-0">Browse</div>
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                @if($chatAktif ?? true)
                                    <div class="flex gap-2 mt-2">
                                        <input type="text" name="pesan" value="{{ old('pesan') }}" placeholder="Ketik pesan balasan atau pertanyaan..." class="flex-1 min-w-0 rounded-xl border border-slate-300 py-2.5 px-3.5 text-sm focus:ring-2 focus:ring-blue-500 bg-white placeholder:text-slate-400">
        ...
        <div class="relative bg-white rounded-[1.5rem] w-full max-w-3xl mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col animate-fade-in-down">
            <div class="flex items-center justify-between gap-4 px-6 md:px-8 py-4 border-b border-[#E4E7EC] bg-gradient-to-r from-[#F8FAFC] to-[#F1F5F9] shrink-0">
                <div class="flex items-center gap-3.5">
                    <div class="w-10 h-10 rounded-xl bg-[#16324F] text-violet-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20">
                        <i class="fa-solid fa-globe"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold tracking-[0.16em] text-violet-700 uppercase mb-0.5">Admin Panel</p>
                        <h2 class="text-[17px] font-extrabold text-[#101828] leading-tight">Pengajuan Subdomain</h2>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <span class="hidden sm:inline-flex items-center gap-1.5 text-[10.5px] font-bold text-violet-800 bg-violet-50 border border-violet-200 rounded-full px-3 py-1 shrink-0">
                        <span class="w-1.5 h-1.5 rounded-full bg-violet-500"></span> Layanan Subdomain
                    </span>
            ...
            <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar">
                <form method="POST" action="{{ route('admin.pengajuan.storeSubdomain') }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="relative pl-10 mb-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3.5">
                            @include('admin.partials.select-asn', ['prefix' => 'subdomain'])
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Nama Pemohon <span class="text-rose-500">*</span></label>
                                <input type="text" name="data_pengajuan[nama]" value="{{ old('data_pengajuan.nama') }}" required placeholder="Nama lengkap..." class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-violet-500 shadow-sm transition-all">
                            </div>
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">NIP Pemohon</label>
                                <input type="text" inputmode="numeric" name="data_pengajuan[nip]" id="nip-field-subdomain" value="{{ old('data_pengajuan.nip') }}" maxlength="17" placeholder="Masukkan NIP (opsional, maksimal 17 digit)..." class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-violet-500 shadow-sm transition-all">
                                <input type="hidden" name="data_pengajuan[perketat_nip]" id="nip-ketat-val-subdomain" value="0">
                                <label class="inline-flex items-center gap-1.5 mt-1.5 text-[10.5px] font-semibold text-[#475467] cursor-pointer select-none">
                                    <input type="checkbox" id="nip-ketat-subdomain" class="accent-violet-600 w-3 h-3 rounded" onchange="toggleNipKetat(this, 'subdomain')">
                    ...
                    {{-- Step 2: Detail Subdomain & Dokumen --}}
                    <div class="relative pl-10 mb-3">
                        ...
                        <h3 class="text-[14px] font-extrabold text-[#101828] mb-3">Detail Subdomain & Dokumen Persyaratan</h3>
                        <input type="hidden" name="jenis_layanan" value="Pembuatan Subdomain">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Nama Subdomain Usulan <span class="text-rose-500">*</span></label>
                                <input type="text" name="data_pengajuan[domain]" value="{{ old('data_pengajuan.domain') }}" required placeholder="Contoh: dinkes" class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-violet-500 shadow-sm transition-all">
                            </div>
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">IP Address Server Tujuan <span class="text-rose-500">*</span></label>
                                <input type="text" inputmode="decimal" name="data_pengajuan[ip_address]" value="{{ old('data_pengajuan.ip_address') }}" required placeholder="103.10.1.5" class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-violet-500 shadow-sm transition-all">
                            </div>
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Nama Sistem / Aplikasi <span class="text-rose-500">*</span></label>
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <label for="admin-subdomain-upload" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-[#DCE1E8] bg-white hover:border-violet-500 hover:bg-violet-50/40 transition-all px-4 py-2.5 cursor-pointer shadow-sm">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <div class="w-8 h-8 shrink-0 rounded-lg bg-slate-100 group-hover:bg-violet-500 group-hover:text-white flex items-center justify-center text-[#667085] transition-colors shadow-sm">
                                            <i class="fa-solid fa-file-pdf text-[14px]"></i>
                                        </div>
                                        <div class="min-w-0">
                                            <p id="admin-subdomain-file-name" class="text-[12.5px] text-[#101828] font-bold group-hover:text-violet-900 truncate">Klik untuk memilih berkas</p>
                                            <p class="text-[10.5px] text-[#667085] font-medium mt-0.5 truncate">Format PDF &middot; Maksimal 5MB</p>
                                        </div>
                                    </div>
                                    <div class="px-3 py-1.5 bg-slate-50 border border-gray-200 rounded-lg text-[11px] font-bold text-gray-600 group-hover:border-violet-300 shrink-0">Browse</div>
                                    <input id="admin-subdomain-upload" name="file_persyaratan" type="file" class="sr-only" accept=".pdf" required onchange="if(this.files && this.files[0]){document.getElementById('admin-subdomain-file-name').innerText = this.files[0].name; document.getElementById('admin-subdomain-file-name').classList.add('text-emerald-700', 'font-bold')}">
                                </label>
                            </div>
                        </div>
                    </div>
