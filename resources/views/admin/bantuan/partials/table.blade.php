                ...
                @forelse ($pengajuans as $item)
                    ...
                    <tr class="hover:bg-cyan-50/20 transition-colors duration-200">
                        ...
                        <td class="py-4 px-6 text-center space-x-1 whitespace-nowrap">
                            <button type="button" onclick="bukaModalInfo('{{ $item->id }}')" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600 border border-gray-200 transition-colors inline-flex items-center justify-center shadow-sm" title="Lihat Detail Tiket">
                                <i class="fa-regular fa-id-card text-xs"></i>
                            </button>
                            <button type="button" onclick="bukaModalAdmin('{{ $item->id }}')" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:bg-cyan-50 hover:text-cyan-600 border border-gray-200 transition-colors inline-flex items-center justify-center shadow-sm" title="Kelola Tiket & Chat">
                    ...
                    <div id="modal-info-{{ $item->id }}" class="fixed inset-0 z-[100] hidden items-center justify-center">
                        ...
                        <div class="relative bg-white rounded-[2rem] p-8 max-w-md w-full mx-4 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
                            ...
                            <div class="space-y-4 bg-gray-50 p-5 rounded-2xl border border-gray-100">
                                <div>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Kategori Kendala</p>
                                    <p class="text-[13px] font-bold text-rose-600">{{ $dataForm['kategori'] ?? '-' }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Rincian Kendala</p>
                                    <p class="text-[13px] font-medium text-gray-700 leading-relaxed">{{ $dataForm['pesan_kendala'] ?? $dataForm['keterangan'] ?? 'Tidak ada catatan khusus.' }}</p>
                                </div>
                            </div>
                            <div class="mt-6">
                                @if(!empty($item->file_pendukung))
                                    <a href="{{ asset('storage/' . $item->file_pendukung) }}" target="_blank" class="w-full block text-center bg-[#071E3D] hover:bg-[#1F4287] text-white py-3 rounded-xl font-bold text-[13px] transition-all shadow-md">
                                        <i class="fa-solid fa-file-pdf mr-2"></i> Download Lampiran Bukti
                                    </a>
                                @else
                    ...
                    <div id="modal-{{ $item->id }}" class="fixed inset-0 z-[100] hidden items-center justify-center">
                        ...
                        <div class="relative bg-white rounded-[2rem] p-8 max-w-xl w-full mx-4 shadow-2xl overflow-y-auto max-h-[95vh] custom-scrollbar">
                            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                                ...
                                <div>
                                    <h3 class="text-xl font-extrabold text-[#071E3D]">Kelola Tiket Bantuan</h3>
                                    <p class="text-[12px] text-gray-500">Update status penanganan dan balas pesan pengguna.</p>
                                </div>
                            </div>
                            ...
                            <form method="POST" action="{{ route('admin.pengajuan.update', $item->id) }}" enctype="multipart/form-data">
                                ...
                                <div class="bg-gray-50/50 border border-gray-100 p-5 rounded-2xl mb-5">
                                    <h4 class="text-[13px] font-extrabold text-[#071E3D] mb-4 uppercase tracking-wider flex items-center gap-2">
                                        <i class="fa-solid fa-bars-progress text-cyan-500"></i> 1. Update Status & Catatan
                                    </h4>
                                    <div class="space-y-4">
                                        <div>
                                        ...
                                        <div class="mb-4">
                                            <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Upload Lampiran Penanganan / File Bukti (Opsional)</label>
                                        ...
                                        <div>
                                            <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Catatan Penanganan</label>
                                            <textarea name="catatan" rows="2" placeholder="Tuliskan catatan solusi/tindakan perbaikan..." class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-cyan-400 resize-none"></textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-blue-50/30 border border-blue-100 p-5 rounded-2xl mb-8">
                                    ...
                                    @if($chatAktif ?? true)
                                        <div>
                                            <textarea name="pesan" rows="2" placeholder="Ketik balasan atau arahan untuk pelapor..." class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-blue-400 resize-none"></textarea>
                                        </div>
                                    @else
    ...
    <div class="relative bg-white rounded-[2rem] p-8 max-w-2xl w-full mx-4 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
        ...
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-extrabold text-[#071E3D]">Buat Tiket Bantuan</h3>
                    <p class="text-[12px] text-gray-500">Input tiket permohonan bantuan / reset password secara manual.</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('admin.bantuan.store') }}" enctype="multipart/form-data" onsubmit="disableSubmitButton(this)">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                ...
                <div class="col-span-1 md:col-span-2">
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Pilih Pegawai / Akun Terdaftar<span class="text-rose-500">*</span></label>
                    <select name="user_id" required class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-bold text-gray-700 outline-none focus:border-cyan-400 transition-colors cursor-pointer">
                        <option value="">Silakan Pilih Akun Terdaftar</option>
                        @php
                        ...
                        @empty
                            <option value="" disabled>Belum ada data pegawai terdaftar</option>
                        @endforelse
                    </select>
                </div>

                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Kategori Kendala<span class="text-rose-500">*</span></label>
                    <select name="data_pengajuan[kategori]" required class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-bold text-gray-700 outline-none focus:border-cyan-400 transition-colors cursor-pointer">
                        <option value="" disabled selected>Pilih Kategori</option>
                        <option value="Reset Password Email">Reset Password</option>
                    </select>
                </div>

                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Nama Pemohon<span class="text-rose-500">*</span></label>
                    <input type="text" name="data_pengajuan[nama]" required placeholder="Nama lengkap pemohon" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-medium text-gray-700 outline-none focus:border-cyan-400 transition-colors">
                </div>

                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">NIP<span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Email Resmi<span class="text-rose-500">*</span></label>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <select name="jenis_layanan" required class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-bold text-gray-700 outline-none focus:border-cyan-400 transition-colors cursor-pointer">
                        <option value="Pusat Bantuan" selected>Pusat Bantuan</option>
                    </select>
                </div>

                <div class="col-span-1 md:col-span-2 bg-cyan-50/50 border border-cyan-100 rounded-xl p-4">
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Upload Surat Permohonan / Bukti Kendala (PDF)<span class="text-rose-500">*</span></label>
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 py-3.5 rounded-xl font-bold text-white bg-[#071E3D] hover:bg-[#1F4287] transition-colors shadow-lg shadow-blue-900/20">
                    <i class="fa-solid fa-paper-plane mr-1"></i> Simpan Tiket
                </button>
            </div>
        </form>
    </div>
