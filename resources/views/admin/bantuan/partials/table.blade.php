            ...
            <div class="relative bg-white rounded-3xl w-full max-w-6xl mx-4 my-auto shadow-2xl border border-slate-100 overflow-hidden max-h-[92vh] flex flex-col animate-fade-in-down">
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-white shrink-0">
                    <div class="flex items-center gap-3.5 min-w-0">
                        <div class="w-11 h-11 rounded-xl bg-[#16324F] text-rose-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20 shrink-0">
                            <i class="fa-solid fa-headset"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold tracking-[0.16em] text-rose-700 uppercase mb-0.5">Admin Panel</p>
                            <h2 class="text-[16px] font-extrabold text-[#101828] leading-tight truncate">Kelola Tiket Bantuan</h2>
                            <p class="text-[11px] text-slate-500 font-medium mt-0.5">Perbarui progres, unggah solusi, dan balas diskusi pemohon.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-rose-800 bg-rose-50 border border-rose-200 rounded-full px-3 py-1.5 shrink-0">
                        ...
                        <div class="p-5 bg-gradient-to-r from-slate-50 to-slate-100/70 border border-slate-200/80 rounded-2xl flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 shadow-sm">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-slate-500 font-medium mt-0.5 truncate">
                                    NIP: {{ $dataForm['nip'] ?? $item->user->nip ?? '-' }} &middot; {{ $dataForm['instansi'] ?? 'Pusat Bantuan' }}
                                </p>
                            </div>
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
                                <div>
                                    ...
                                    <p class="text-xs font-semibold text-slate-700 break-all max-w-[220px]">{{ $dataForm['email_reset'] ?? $dataForm['email'] ?? '-' }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Kategori</p>
                                    <span class="px-3.5 py-1.5 bg-rose-100 text-rose-800 rounded-xl text-xs font-bold shadow-sm inline-block">{{ $dataForm['kategori'] ?? '-' }}</span>
                                </div>
                            </div>

                            @if(!empty($filePemohon))
                                <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'pendukung']) }}" target="_blank" class="px-4 py-2.5 bg-white border border-slate-200 hover:border-rose-500 hover:bg-rose-50/30 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2.5 shadow-sm transition-all shrink-0">
                                    <i class="fa-solid fa-file-pdf text-sm text-rose-600"></i> Lampiran
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                <div class="space-y-4 flex-1">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="catatan" rows="4" placeholder="Tuliskan catatan solusi/tindakan perbaikan..." class="w-full rounded-xl border border-slate-300 p-3.5 text-sm focus:ring-2 focus:ring-blue-500 bg-white placeholder:text-slate-400">{{ old('catatan') }}</textarea>
                                    </div>

                                    <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Upload Bukti Penanganan (PDF)</label>
                                        <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white hover:border-rose-500 hover:bg-rose-50/40 transition-all px-3 py-2.5 cursor-pointer">
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                <div class="w-8 h-8 shrink-0 rounded-lg bg-white group-hover:bg-rose-600 group-hover:text-white border border-slate-200 group-hover:border-rose-600 flex items-center justify-center text-slate-500 transition-colors shadow-sm">
                                                    <i class="fa-solid fa-cloud-arrow-up text-[13px]"></i>
                                                </div>
                                                <div class="min-w-0">
                                                    <p id="admin-hasil-name-{{ $item->id }}" class="text-[12px] text-[#101828] font-bold group-hover:text-rose-900 truncate">
                                                        @if($item->file_hasil)
                                                            File Hasil Tersedia
                                                            <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'hasil']) }}" target="_blank" class="inline-flex items-center gap-1 ml-1.5 text-rose-700 bg-rose-100 hover:bg-rose-200 px-2 py-0.5 rounded-md text-[10px] font-bold transition-colors">
                                                                <i class="fa-solid fa-eye text-[9px]"></i> Lihat
                                                            </a>
                                                        @else
                                                            Pilih Berkas PDF Bukti
                                                        @endif
                                                    </p>
                                                    <p class="text-[10px] text-slate-500">Dokumen hasil penanganan</p>
                                                </div>
                                            </div>
                                            <div class="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[10.5px] font-bold text-gray-600 group-hover:border-rose-300 shrink-0">Browse</div>
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                @if($chatAktif ?? true)
                                    <div class="flex gap-2 mt-2">
                                        <input type="text" name="pesan" value="{{ old('pesan') }}" placeholder="Ketik balasan atau arahan..." class="flex-1 min-w-0 rounded-xl border border-slate-300 py-2.5 px-3.5 text-sm focus:ring-2 focus:ring-blue-500 bg-white placeholder:text-slate-400">
