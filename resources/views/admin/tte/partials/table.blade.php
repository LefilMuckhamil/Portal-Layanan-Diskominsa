<div data-ajax-table class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 overflow-hidden flex flex-col mt-6">
    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-[12px] text-gray-400 font-medium mt-1">Kelola data dan perbarui status progres pengajuan TTE.</p>
        </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center">
                            ...
                            <h3 class="font-bold text-[14px] text-gray-600">Belum ada data pengajuan TTE</h3>
                        </td>
                    </tr>
                @endforelse
            </tbody>
                ...
                <div class="flex items-center justify-between gap-4 px-6 md:px-8 py-4 border-b border-[#E4E7EC] bg-gradient-to-r from-[#F8FAFC] to-[#F1F5F9] shrink-0">
                    <div class="flex items-center gap-3.5">
                        ...
                        <div>
                            ...
                            <h2 class="text-[17px] font-extrabold text-[#101828] leading-tight">Kelola Pengajuan TTE</h2>
                        </div>
                    </div>
                ...
                <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar flex-1 flex flex-col">
                    <form method="POST" action="{{ route('admin.pengajuan.update', $item->id) }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)" class="flex-1 flex flex-col justify-between">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-stretch flex-1">
                            ...
                            {{-- KOLOM 1: INFORMASI PEMOHON & DOKUMEN --}}
                                ...
                                <div>
                                    ...
                                    <div class="bg-white border border-slate-200 rounded-xl p-3.5 space-y-2.5 shadow-sm">
                                        <div>
                                            ...
                                            <p class="text-[13px] font-extrabold text-[#071E3D]">{{ $dataForm['nama'] ?? $dataForm['nama_lengkap'] ?? $item->user->name ?? '-' }}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <p class="text-[9.5px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Email Aktif</p>
                                            <p class="text-[12px] font-bold text-[#071E3D] break-all">{{ $dataForm['email'] ?? '-' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-[9.5px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Alamat Domisili</p>
                                            <p class="text-[12px] font-bold text-[#071E3D]">{{ $dataForm['alamat'] ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div class="flex items-center justify-between mb-3.5">
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <h3 class="text-[13.5px] font-extrabold text-[#101828]">Status & Hasil TTE</h3>
                                        </div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                            ...
                                            <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-2 rounded-xl border-2 border-dashed border-[#DCE1E8] bg-white hover:border-blue-500 hover:bg-blue-50/40 transition-all px-3 py-2 cursor-pointer shadow-sm">
                                                ...
                                                <div class="px-2.5 py-1 bg-slate-50 border border-gray-200 rounded-lg text-[10.5px] font-bold text-gray-600 group-hover:border-blue-300 shrink-0">Browse</div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="catatan" placeholder="Tuliskan catatan progres yang akan muncul di E-Tracking..." class="w-full bg-white border border-slate-200 rounded-xl p-3 text-[12px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-blue-400 resize-none h-28 shadow-sm">{{ old('catatan') }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    {{-- MODAL TAMBAH PENGAJUAN (CREATE DENGAN BORDER INPUT LENGKAP) --}}
    <div id="modal-create" class="fixed inset-0 z-[150] hidden items-center justify-center">
        ...
        <div class="relative bg-white rounded-[1.5rem] w-full max-w-3xl mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col animate-fade-in-down">
            ...
            <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar">
                <form method="POST" action="{{ route('admin.pengajuan.storeTte') }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="relative pl-10 mb-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3.5">
                            @include('admin.partials.select-asn', ['prefix' => 'tte'])
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">Nama Lengkap <span class="text-rose-500">*</span></label>
                                <input type="text" name="data_pengajuan[nama]" value="{{ old('data_pengajuan.nama') }}" required placeholder="Contoh: Budi Santoso, S.Kom" class="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-blue-500 shadow-sm transition-all">
                            </div>
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">NIP</label>
                                <input type="text" inputmode="numeric" name="data_pengajuan[nip]" id="nip-field-tte" value="{{ old('data_pengajuan.nip') }}" maxlength="17" placeholder="Masukkan NIP (opsional, maksimal 17 digit)..." class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-blue-500 shadow-sm transition-all">
                                <input type="hidden" name="data_pengajuan[perketat_nip]" id="nip-ketat-val-tte" value="0">
                                <label class="inline-flex items-center gap-1.5 mt-1.5 text-[10.5px] font-semibold text-[#475467] cursor-pointer select-none">
                                    <input type="checkbox" id="nip-ketat-tte" class="accent-blue-600 w-3 h-3 rounded" onchange="toggleNipKetat(this, 'tte')">
                                    Perketat NIP (18 digit wajib)
                                </label>
                            </div>
                            <div>
                                <label class="block text-[11.5px] font-bold text-[#344054] mb-1">NIK <span class="text-rose-500">*</span></label>
                                <input type="text" inputmode="numeric" name="data_pengajuan[nik]" value="{{ old('data_pengajuan.nik') }}" required maxlength="16" placeholder="Masukkan NIK 16 digit..." class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-blue-500 shadow-sm transition-all">
                            </div>
                    ...
                    {{-- Step 2: Kontak & Dokumen Persyaratan --}}
                    <div class="relative pl-10 mb-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <input type="text" name="data_pengajuan[alamat]" value="{{ old('data_pengajuan.alamat') }}" required placeholder="Masukkan alamat domisili lengkap..." class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-[12.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] outline-none focus:border-blue-500 shadow-sm transition-all">
                            </div>
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <label for="admin-tte-upload" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-[#DCE1E8] bg-white hover:border-blue-500 hover:bg-blue-50/40 transition-all px-4 py-2.5 cursor-pointer shadow-sm">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <p id="admin-tte-name" class="text-[10.5px] text-[#667085] font-medium mt-0.5 truncate">Surat Rekomendasi/KTP (Maks 5MB)</p>
                                        </div>
                                    </div>
                                    <div class="px-3 py-1.5 bg-slate-50 border border-gray-200 rounded-lg text-[11px] font-bold text-gray-600 group-hover:border-blue-300 shrink-0">Browse</div>
                                    <input id="admin-tte-upload" name="file_pendukung" type="file" class="sr-only" accept=".pdf" required onchange="if(this.files && this.files[0]){document.getElementById('admin-tte-name').innerText = this.files[0].name; document.getElementById('admin-tte-name').classList.add('text-emerald-700', 'font-bold')}">
                                </label>
                            </div>
                        </div>
                    </div>
