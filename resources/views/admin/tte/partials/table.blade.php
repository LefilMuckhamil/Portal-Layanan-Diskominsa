                    ...
                    <tr class="hover:bg-cyan-50/20 transition-colors duration-200">
                        ...
                        <td class="py-4 px-6">
                            ...
                            @if(!empty($dataForm['no_hp']))
                                <p class="text-[11px] text-gray-500 font-medium mt-0.5">
                                    <i class="fa-brands fa-whatsapp text-green-500 mr-1"></i>{{ \App\Support\PhoneNumber::local($dataForm['no_hp'] ?? '') }}
                                </p>
                            @endif
                        </td>
            ...
            <div class="relative bg-white rounded-3xl w-full max-w-6xl mx-4 my-auto shadow-2xl border border-slate-100 overflow-hidden max-h-[92vh] flex flex-col animate-fade-in-down">
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-white shrink-0">
                    <div class="flex items-center gap-3.5 min-w-0">
                        <div class="w-11 h-11 rounded-xl bg-[#16324F] text-blue-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20 shrink-0">
                            <i class="fa-solid fa-file-signature"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold tracking-[0.16em] text-blue-700 uppercase mb-0.5">Admin Panel</p>
                            <h2 class="text-[16px] font-extrabold text-[#101828] leading-tight truncate">Kelola Pengajuan TTE</h2>
                            <p class="text-[11px] text-slate-500 font-medium mt-0.5">Perbarui progres, unggah berkas hasil TTE, dan balas diskusi pemohon.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-blue-800 bg-blue-50 border border-blue-200 rounded-full px-3 py-1.5 shrink-0">
                ...
                <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
                    <form method="POST" action="{{ route('admin.pengajuan.update', $item->id) }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                        ...
                        <div class="p-5 bg-gradient-to-r from-slate-50 to-slate-100/70 border border-slate-200/80 rounded-2xl flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 shadow-sm">
                            <div class="min-w-0">
                                <p class="text-base font-bold text-slate-800 truncate">{{ $dataForm['nama'] ?? $dataForm['nama_lengkap'] ?? $item->user->name ?? 'Pemohon' }}</p>
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
                                ...
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">NIK</p>
                                    <span class="px-3.5 py-1.5 bg-blue-100 text-blue-800 rounded-xl text-xs font-bold shadow-sm inline-block">{{ $dataForm['nik'] ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                <div class="space-y-4 flex-1">
                                    ...
                                    <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Upload Berkas Hasil TTE (PDF)</label>
                                        <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white hover:border-blue-500 hover:bg-blue-50/40 transition-all px-3 py-2.5 cursor-pointer">
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <p class="text-[10px] text-slate-500">Dokumen tanda tangan elektronik</p>
                                                </div>
                                            </div>
