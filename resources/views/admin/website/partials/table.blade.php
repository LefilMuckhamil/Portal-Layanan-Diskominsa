            ...
            <div class="relative bg-white rounded-3xl w-full max-w-6xl mx-4 my-auto shadow-2xl border border-slate-100 overflow-hidden max-h-[92vh] flex flex-col animate-fade-in-down">
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-white shrink-0">
                    <div class="flex items-center gap-3.5 min-w-0">
                        <div class="w-11 h-11 rounded-xl bg-[#16324F] text-indigo-400 flex items-center justify-center text-lg shadow-md shadow-[#16324F]/20 shrink-0">
                            <i class="fa-solid fa-globe"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold tracking-[0.16em] text-indigo-700 uppercase mb-0.5">Admin Panel</p>
                            <h2 class="text-[16px] font-extrabold text-[#101828] leading-tight truncate">Kelola Pengajuan Website</h2>
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-indigo-800 bg-indigo-50 border border-indigo-200 rounded-full px-3 py-1.5 shrink-0">
                    ...
                    <form method="POST" action="{{ route('admin.pengajuan.update', $item->id) }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                        ...
                        <div class="p-5 bg-gradient-to-r from-slate-50 to-slate-100/70 border border-slate-200/80 rounded-2xl flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 shadow-sm">
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
                                ...
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Nama Website</p>
                                    <span class="px-3.5 py-1.5 bg-indigo-100 text-indigo-800 rounded-xl text-xs font-bold shadow-sm inline-block">{{ $dataForm['nama_website'] ?? $dataForm['domain'] ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            @if(!empty($filePemohon))
                                <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'pendukung']) }}" target="_blank" class="px-4 py-2.5 bg-white border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50/30 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2.5 shadow-sm transition-all shrink-0">
                                    <i class="fa-solid fa-file-pdf text-sm text-indigo-600"></i> Surat Permohonan
                            ...
                            <div class="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                                ...
                                <div class="space-y-4 flex-1">
                                    ...
                                    <div id="hasil-upload-{{ $item->id }}" class="{{ $item->status === 'Selesai' ? '' : 'hidden' }}">
                                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Upload Dokumen Hasil / Akses Web (PDF)</label>
                                        <label for="admin-hasil-upload-{{ $item->id }}" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-white hover:border-indigo-500 hover:bg-indigo-50/40 transition-all px-3 py-2.5 cursor-pointer">
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                <div class="w-8 h-8 shrink-0 rounded-lg bg-white group-hover:bg-indigo-600 group-hover:text-white border border-slate-200 group-hover:border-indigo-600 flex items-center justify-center text-slate-500 transition-colors shadow-sm">
                                                    <i class="fa-solid fa-cloud-arrow-up text-[13px]"></i>
                                                </div>
                                                <div class="min-w-0">
                                                    <p id="admin-hasil-name-{{ $item->id }}" class="text-[12px] text-[#101828] font-bold group-hover:text-indigo-900 truncate">
                                                        @if($item->file_hasil)
                                                            File Hasil Tersedia
                                                            <a href="{{ route('dokumen.unduh', ['pengajuan' => $item->id, 'jenis' => 'hasil']) }}" target="_blank" class="inline-flex items-center gap-1 ml-1.5 text-indigo-700 bg-indigo-100 hover:bg-indigo-200 px-2 py-0.5 rounded-md text-[10px] font-bold transition-colors">
                                            ...
                                            <div class="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[10.5px] font-bold text-gray-600 group-hover:border-indigo-300 shrink-0">Browse</div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <label for="admin-website-upload" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-[#DCE1E8] bg-white hover:border-indigo-500 hover:bg-indigo-50/40 transition-all px-4 py-2.5 cursor-pointer shadow-sm">
                                    ...
                                    <input id="admin-website-upload" name="file_pendukung" type="file" class="sr-only" accept=".pdf" required onchange="if(this.files && this.files[0]){document.getElementById('admin-website-file-name').innerText = this.files[0].name; document.getElementById('admin-website-file-name').classList.add('text-emerald-700', 'font-bold')}">
                                </label>
                            </div>
                        </div>
                    </div>
