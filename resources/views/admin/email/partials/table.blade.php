<div data-ajax-table class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 overflow-hidden flex flex-col mt-6">
    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-lg font-extrabold text-[#071E3D]">Daftar Pengajuan Email Resmi</h3>
                ...
                @forelse ($pengajuans as $item)
                    ...
                    <tr class="hover:bg-cyan-50/20 transition-colors duration-200">
                        ...
                        <td class="py-4 px-6">
                            ...
                            @if(!empty($dataForm['usulan_email']))
                                <p class="text-[11px] text-cyan-600 font-medium mt-0.5">
                                    {{ $dataForm['usulan_email'] }}
                                </p>
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center">
                            ...
                            <h3 class="font-bold text-[14px] text-gray-600">Belum ada data pengajuan email</h3>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    {{-- MODAL TAMBAH PENGAJUAN (CREATE TANPA UPLOAD DOKUMEN) --}}
    <div id="modal-create" class="fixed inset-0 z-[150] hidden items-center justify-center">
        ...
        <div class="relative bg-white rounded-[1.5rem] w-full max-w-3xl mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col animate-fade-in-down">
            ...
            <div class="px-6 md:px-8 py-5 overflow-y-auto custom-scrollbar">
                <form method="POST" action="{{ route('admin.pengajuan.storeEmail') }}" enctype="multipart/form-data" data-no-ajax onsubmit="return konfirmasiTolak(this)">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="relative pl-10 mb-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3.5">
                            @include('admin.partials.select-asn', ['prefix' => 'email'])
                            <div>
                    ...
                    <div class="relative pl-10 mb-3">
                        ...
                        <h3 class="text-[14px] font-extrabold text-[#101828] mb-3">Detail Email</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <label for="admin-email-upload" class="group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-[#DCE1E8] bg-white hover:border-sky-500 hover:bg-sky-50/40 transition-all px-4 py-2.5 cursor-pointer shadow-sm">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <p id="admin-email-file-name" class="text-[12.5px] text-[#101828] font-bold group-hover:text-sky-900 truncate">Klik untuk memilih berkas</p>
                                    ...
                                    <input id="admin-email-upload" name="file_pendukung" type="file" class="sr-only" accept=".pdf" required onchange="if(this.files && this.files[0]){document.getElementById('admin-email-file-name').innerText = this.files[0].name; document.getElementById('admin-email-file-name').classList.add('text-emerald-700', 'font-bold')}">
                                </label>
                            </div>
                        </div>
                    </div>
