            ...
            <form method="GET" action="{{ route('admin.cloud.index') }}" class="flex gap-3">
                ...
                <div class="relative">
                    ...
                    <select name="status" onchange="this.form.submit()" class="pl-8 pr-4 py-2 bg-gray-50 border border-gray-100 rounded-lg text-[12px] font-bold text-gray-600 outline-none cursor-pointer">
                        ...
                        @foreach(['Pending' => 'Pending', 'Verifikasi Doc' => 'Verifikasi Dokumen', 'Proses Development' => 'Proses Pembuatan', 'Selesai' => 'Selesai / Aktif', 'Ditolak' => 'Ditolak'] as $val => $label)
                ...
                @forelse ($pengajuans as $item)
                    ...
                    <tr class="hover:bg-cyan-50/20 transition-colors duration-200">
                        ...
                        <td class="py-4 px-6">
                            ...
                            {{-- Menampilkan Email di atas Penyimpanan --}}
                        ...
                        <td class="py-4 px-6">
                            @php
                                $badgeColor = match($item->status) {
                                    ...
                                    'Verifikasi Doc', 'Proses Development', 'Proses' => 'bg-blue-50 text-blue-600 border-blue-100',
                            ...
                            <span class="px-3 py-1.5 rounded-lg border text-[10px] font-extrabold uppercase tracking-wider {{ $badgeColor }}">
                                {{ $item->status == 'Proses Development' || $item->status == 'Proses' ? 'Proses Pembuatan' : $item->status }}
                            </span>
                        </td>
                    ...
                    <div id="modal-{{ $item->id }}" class="fixed inset-0 z-[100] hidden items-center justify-center">
                        ...
                        <div class="relative bg-white rounded-[2rem] p-8 max-w-xl w-full mx-4 shadow-2xl overflow-y-auto max-h-[95vh] custom-scrollbar">
                            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                                ...
                                <div>
                                    <h3 class="text-xl font-extrabold text-[#071E3D]">Kelola Permohonan Cloud</h3>
                                    <p class="text-[12px] text-gray-500">Update progres, serahkan berkas/akses, dan balas pesan.</p>
                                </div>
                            </div>
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <select name="status" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] text-[#071E3D] font-bold focus:border-cyan-400 outline-none transition-colors">
                                                @foreach(['Pending' => 'PENDING', 'Verifikasi Doc' => 'VERIFIKASI DOC', 'Proses Development' => 'PROSES PEMBUATAN', 'Selesai' => 'SELESAI / AKTIF', 'Ditolak' => 'DITOLAK'] as $val => $label)
                                                    <option value="{{ $val }}" @selected($item->status == $val)>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="mb-4">
                                            <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Upload Dokumen Hasil / Akses Cloud (Jika Selesai)</label>
    ...
    <div class="relative bg-white rounded-[2rem] p-8 max-w-2xl w-full mx-4 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
        ...
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-extrabold text-[#071E3D]">Tambah Permohonan Cloud</h3>
                    <p class="text-[12px] text-gray-500">Buat permohonan cloud storage baru secara manual untuk ASN.</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('admin.pengajuan.storeCloud') }}" enctype="multipart/form-data" onsubmit="disableSubmitButton(this)">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                ...
                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Nama Penanggung Jawab<span class="text-rose-500">*</span></label>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="data_pengajuan[instansi]" required placeholder="Contoh: Dinas Komunikasi dan Informatika" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-medium text-gray-700 outline-none focus:border-cyan-400 transition-colors">
                </div>

                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Email Resmi (Aktivasi)<span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-[12px] font-bold text-[#071E3D] mb-2">Kapasitas Penyimpanan<span class="text-rose-500">*</span></label>
                    <select name="data_pengajuan[kapasitas]" required class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-bold text-gray-700 outline-none focus:border-cyan-400 transition-colors cursor-pointer">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <select name="jenis_layanan" required class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-[13px] font-bold text-gray-700 outline-none focus:border-cyan-400 transition-colors cursor-pointer">
                        <option value="Cloud Government" selected>Cloud Government</option>
                    </select>
                </div>
