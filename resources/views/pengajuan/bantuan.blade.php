            ...
            <form action="{{ route('pengajuan.bantuan.store') }}" method="POST" enctype="multipart/form-data" onsubmit="disableSubmitButton(this)">
                ...
                <div class="relative dk-rail pl-11">
                    ...
                    <div class="mb-5">
                        ...
                        <select name="data_pengajuan[kategori_bantuan_id]" required class="dk-input w-full px-3.5 py-2.5 text-[13.5px] text-[#101828] font-bold appearance-none cursor-pointer">
                            <option value="" disabled {{ old('data_pengajuan.kategori_bantuan_id') ? '' : 'selected' }}>Pilih Kategori Kendala</option>
                            @foreach ($kategoris as $kategori)
                                <option value="{{ $kategori->id }}" {{ old('data_pengajuan.kategori_bantuan_id') == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="relative pl-11">
                    ...
                    <p class="text-[12.5px] text-[#667085] font-medium mb-6">Informasi akun yang bermasalah dan unggah berkas pengajuan.</p>

                    <div class="mb-6">
                        <label class="block text-[12.5px] font-bold text-[#344054] mb-1.5">Email yang Ingin Direset</label>
                        <input type="email" name="data_pengajuan[email_reset]" value="{{ old('data_pengajuan.email_reset') }}" required placeholder="email@acehbaratkab.go.id" class="dk-input w-full px-3.5 py-2.5 text-[13.5px] text-[#101828] font-medium placeholder:text-[#98A2B3]">
                        <p class="text-[11px] text-[#667085] font-medium mt-1.5 ml-1">Pastikan alamat email resmi yang dimasukkan sudah benar.</p>
                    </div>

                    <div class="mb-6">
                        <label class="block text-[12.5px] font-bold text-[#344054] mb-1.5">Deskripsi Kendala</label>
                        <textarea name="data_pengajuan[deskripsi_kendala]" rows="3" placeholder="Jelaskan kendala yang dialami secara singkat (opsional)" class="dk-input w-full px-3.5 py-2.5 text-[13.5px] text-[#101828] font-medium placeholder:text-[#98A2B3] resize-none">{{ old('data_pengajuan.deskripsi_kendala') }}</textarea>
                    </div>

                    <div class="mb-4">
                        <a href="{{ asset('templates/template_permohonan_bantuan.docx') }}" class="group flex items-center justify-between gap-4 rounded-xl border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100 transition-all px-5 py-4 shadow-sm">
                    ...
                    <div class="mb-2">
                        <label class="block text-[12.5px] font-bold text-[#344054] mb-1.5">Upload Surat Permohonan Bantuan (PDF)</label>
