...
<body class="min-h-screen w-full bg-gradient-to-br from-[#071E3D] via-[#0D2B52] to-[#1F4287] flex items-center justify-center p-4 md:p-6 relative selection:bg-cyan-300 selection:text-[#071E3D]">
    ...
    <div class="animate-card-down bg-white w-full max-w-5xl rounded-3xl shadow-[0_25px_70px_-15px_rgba(0,0,0,0.5)] overflow-hidden grid grid-cols-1 md:grid-cols-2 relative z-10 my-auto transition-all">
        ...
        <div class="w-full relative overflow-hidden group shrink-0 rounded-2xl p-3 md:p-4 min-h-[220px] md:min-h-0">
        ...
        <div class="animate-form-down w-full flex flex-col justify-center p-6 md:p-8 bg-white">
            <div class="w-full max-w-md mx-auto flex flex-col">
                ...
                <div class="flex flex-col items-center mb-4">
                    <img src="{{ asset('image/kominsa_biru.png') }}" alt="Logo" class="h-16 sm:h-20 md:h-24 w-auto object-contain drop-shadow-sm mb-3 hover:scale-105 transition-transform duration-300">
                ...
                <div>
                    @if (session('sukses'))
                        <div class="mb-3 p-3 bg-green-50 border border-green-100 text-green-600 text-xs md:text-sm rounded-xl font-bold flex items-center gap-2 shadow-sm">
                            <i class="fa-solid fa-circle-check text-sm"></i> {{ session('sukses') }}
                        </div>
                    @endif

                    @if (session('info'))
                        <div class="mb-3 p-3 bg-amber-50 border border-amber-100 text-amber-600 text-xs md:text-sm rounded-xl font-bold flex items-center gap-2 shadow-sm">
                            <i class="fa-solid fa-circle-info text-sm"></i> {{ session('info') }}
                        </div>
                    @endif

                    @if (session('error'))
                        <div class="mb-3 p-3 bg-rose-50 border border-rose-100 text-rose-600 text-xs md:text-sm rounded-xl font-bold flex items-center gap-2 shadow-sm">
                            <i class="fa-solid fa-circle-exclamation text-sm"></i> {{ session('error') }}
                        </div>
                    @endif
                    
                    @if ($errors->any())
                        <div class="mb-3 p-3 bg-red-50 border border-red-100 text-red-500 text-xs md:text-sm rounded-xl font-bold flex items-center gap-2 shadow-sm">
                            <i class="fa-solid fa-circle-exclamation text-sm"></i> {{ $errors->first('email') }}
                        </div>
                    @endif

                    <form action="{{ url('/login') }}" method="POST" autocomplete="off" class="space-y-3.5">
                        @csrf
